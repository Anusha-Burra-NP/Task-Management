...
const updateTask = async (req, res) => {
    try {
        ...
        res.json({ message: "Task updated successfully", "updatedTask": updatedTask });
    } catch (error) {
        res.status(500).json({ message: "Server Error", error: error.message });
    }
};
...
const getDashboardData = async (req, res) => {
    try {
        ...
        const overdueTasks = await Task.countDocuments({
            status: { $ne: "Completed" },
            dueDate: { $lt: new Date() },
        }); //compares tasks date with today's date, and which are eralier will be added

        //Ensure all possible statusses are included
        ...
        //Ensure all property levels are included
